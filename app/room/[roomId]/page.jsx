...
import ReactionRenderer, {
  ReactionBar,
} from "../../components/ReactionRenderer";
...
const RoundTimer = ({ timeLeft, maxTime }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div
        className={`relative w-20 h-20 ${danger ? "animate-pulse-fast" : ""}`}
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span
            className={`font-display font-bold text-xl ${danger ? "text-red-400" : "text-white"}`}
          >
            {timeLeft}
          </span>
        </div>
    ...
    >
      ...
      >
        ...
        <p className="text-sm text-gray-400 mb-4">
          Score for <span className="font-semibold text-white">{username}</span>
        </p>
        <div className="grid grid-cols-2 gap-3 mb-4">
          {[
            ...
            <div
              key={s.label}
              className="bg-dark-200 rounded-lg p-3 text-center"
            >
      ...
      >
          ...
          >
            <p className="text-sm text-purple-300">
              ...
              <span className="font-bold text-purple-200">
                {result.crowdWinner.username}
              </span>{" "}
              is the People's Champ!
            </p>
            <p className="text-xs text-purple-400/70 mt-0.5">
              The audience disagreed with the AI judge
            </p>
          </motion.div>
        )}
        ...
        <div className="space-y-3 mb-6">
          {result.finalScores?.map((s) => (
            <div
              key={s.userId}
              className="flex items-center justify-between p-3 bg-dark-200 rounded-xl"
            >
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            <span
              className={`badge ${mode === "roast" ? "bg-orange-900/40 text-orange-400" : "bg-blue-900/40 text-blue-400"}`}
            >
          ...
          <p className="font-semibold text-sm text-gray-200 max-w-md line-clamp-1">
            ...
            <div className="p-3 border-t border-white/10 space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && !e.shiftKey && sendMessage(false)
                  }
                  placeholder={
                    gameStatus === "active"
                ...
                <button
                  ...
                  disabled={!inputMsg.trim() || gameStatus === "idle"}
        ...
        <div className="w-64 flex flex-col gap-4 hidden md:flex">
          ...
          {gameStatus === "active" && (
            <div className="card text-center">
              ...
              <p
                className={`text-xs mt-3 font-semibold ${isMyTurn ? "text-brand-400" : "text-gray-500"}`}
              >
          ...
          <div className="card">
            ...
            <div className="space-y-2">
              {players.map((p) => (
                <div
                  key={p.userId}
                  className={`flex items-center gap-2.5 p-2 rounded-lg ${currentTurn === p.userId?.toString() ? "bg-brand-600/10 border border-brand-600/30" : "bg-dark-200"}`}
                >
                  ...
                  {currentTurn === p.userId?.toString() && (
                    <span className="text-xs text-brand-400">⚔️</span>
                  )}
                </div>
              ))}
              {players.length < 2 && (
                <div className="flex items-center gap-2.5 p-2 rounded-lg bg-dark-200 opacity-50">
                  <div className="w-8 h-8 rounded-full border-2 border-dashed border-gray-600 flex items-center justify-center text-gray-600 text-xs">
                    ?
                  </div>
