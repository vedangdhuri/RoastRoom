...
const XPProgressBar = ({ xp, level }) => {
  ...
  const progressPattern = Math.min(
...
const BADGE_DESC = {
  ...
  five_wins: "5 Wins",
  ten_wins: "10 Wins",
  roast_king: "Roast Master",
  debate_legend: "Debate Scholar",
  high_scorer: "Max Score",
};
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="card border-brand-500/20 bg-brand-900/10">
          ...
          <div className="flex items-end gap-3 mb-2">
            ...
            <span className="text-brand-400 font-medium mb-1 border px-2 py-0.5 rounded border-brand-500/30 bg-brand-500/10">
              Level
            </span>
          </div>
        ...
        {user.badges?.length > 0 ? (
          <div className="flex flex-wrap gap-3">
            {user.badges.map((b) => (
              <div
                key={b}
                className="flex items-center gap-2 px-3 py-2 bg-dark-200 border border-white/5 rounded-lg group hover:border-brand-500/50 transition-colors"
              >
          ...
          <p className="text-sm text-gray-500">
            Play matches and win to earn badges.
          </p>
        )}
      </div>
          ...
          {loading ? (
            ...
            history.map((match) => {
              ...
              return (
                <div
                  key={match._id}
                  className="flex items-center justify-between p-4 bg-dark-200 rounded-xl border border-white/5"
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-2xl ${isWinner ? "drop-shadow-[0_0_8px_rgba(34,197,94,0.5)]" : "grayscale"}`}
                    >
                      {isWinner ? "🏆" : "💀"}
                    </span>
                    <div>
